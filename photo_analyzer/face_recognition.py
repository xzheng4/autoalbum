...
class FaceRecognizer:
    ...
    def unregister_person(self, name: str) -> bool:
        """删除某人的所有人脸记录"""
        ...
        new_names = []
        new_encodings = []
        for n, e in zip(self.known_face_names, self.known_face_encodings):
            if n != name:
                new_names.append(n)
    ...
    def scan_faces_directory(self) -> Dict[str, bool]:
        """
        扫描 faces 目录，自动注册已知成员
        假设目录结构为：faces/张三/photo1.jpg, faces/张三/photo2.jpg
        ...
        for person_dir in FACES_DIR.iterdir():
            if person_dir.is_dir():
                ...
                if image_paths:
                    results[person_name] = self.register_person(
                        person_name,
